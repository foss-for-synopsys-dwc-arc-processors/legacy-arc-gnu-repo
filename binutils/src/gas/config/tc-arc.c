...
arc700_special_symtype current_special_sym_flag;
...
#if 0
	      if (!mach_a4 && current_special_sym_flag == GOT_TYPE)
 ...
 */
const struct attributes ac_syntaxclass[] = {
...
const struct attributes ac_syntaxclassmodifier[] = {
...
static int
arc_get_sda_reloc (insn, compact_insn_16)
     arc_insn insn;
     int compact_insn_16;
{
  ...
  if ((int) fixP->fx_r_type >= (int) BFD_RELOC_UNUSED)
      ...
      else if (operand->fmt == 'L')
	{
	  assert ((operand->flags & ARC_OPERAND_LIMM) != 0
		  && operand->bits == 32
		  && operand->shift == 32);
	  fixP->fx_r_type = (mach_a4) ? BFD_RELOC_32 : BFD_RELOC_ARC_32_ME;
	}
      /* ARCtangent-A5 21-bit (shift by 2) PC-relative relocation. Used for
         bl<cc> instruction */
      else if (!mach_a4 && (operand->fmt == 'h'))
	{
	  assert ((operand->flags & ARC_OPERAND_RELATIVE_BRANCH) != 0
		  && operand->bits == 19
		  && operand->shift == 18);
