...
next_pc(CORE_ADDR pc, CORE_ADDR *fall_thru, CORE_ADDR *target)
{
    ...
    *fall_thru = pc + instr.instructionLen;
     ...
     */
    {
        ...
        regcache_cooked_read(current_regcache, ARC_LP_START_REGNUM, (gdb_byte*) &lp_start);
        regcache_cooked_read(current_regcache, ARC_LP_END_REGNUM,   (gdb_byte*) &lp_end);
        regcache_cooked_read(current_regcache, ARC_LP_COUNT_REGNUM, (gdb_byte*) &lp_count);
        regcache_cooked_read(current_regcache, ARC_STATUS32_REGNUM, (gdb_byte*) &status32);
                                ...
                                gdb_byte        *buf)
{
    ...
    regcache_cooked_read (current_regcache, ARC_ORIG_R8_REGNUM, (gdb_byte*) &orig_r8);

    if (regno == ARC_ILINK1_REGNUM ||
        regno == ARC_ILINK2_REGNUM ||
        regno == ARC_ERET_REGNUM)
    {
        regcache_cooked_read (current_regcache, ARC_RET_REGNUM, (gdb_byte*) &ret);
 ...
 * with insert_breakpoints_p = 0 after the step
  */
static void
arc_software_single_step(enum target_signal ignore, int insert_breakpoints)
{
    /* these variables must hold their values between the two calls, so they
     * are static
     */
    static Boolean                two_breakpoints;
...
#ifdef ARC_DEBUG
    {
        unsigned int efa, ret = 0;

        regcache_cooked_read(current_regcache, ARC_EFA_REGNUM, (gdb_byte*) &efa);
//      regcache_cooked_read(current_regcache, ARC_RET_REGNUM, (gdb_byte*) &ret);

        DEBUG("--- arc_software_single_step, efa = %x, ret = %x, (%s)\n", efa, ret,
              (insert_breakpoints ? "add" : "remove"));
    }
#endif
    ...
    if (insert_breakpoints)
    {
        ...
        DEBUG("two = %d, fall_thru = %x, branch_target = %x\n", two_breakpoints, fall_thru, branch_target);

        if (two_breakpoints && branch_target == pc)
        {
            warning ("Can not single-step branch-to-self or single instruction zero overhead loop,\n"
                     "    Stepping across it.");
        ...
        result = target_insert_breakpoint (&bp_tgt1);
        if (result != 0)
            warning ("Can not set breakpoint at 0x%08X\n",  (unsigned int) bp_tgt1.placed_address);

        if (two_breakpoints)
        {
            bp_tgt2.placed_address = branch_target;

            result = target_insert_breakpoint (&bp_tgt2);
            if (result != 0)
                warning ("Can not set breakpoint at 0x%08X\n",  (unsigned int) bp_tgt2.placed_address);
        }
    }
    else
    {
        result = target_remove_breakpoint (&bp_tgt1);
        if (result != 0)
            warning ("Can not remove breakpoint at 0x%08X\n",  (unsigned int) bp_tgt1.placed_address);

        if (two_breakpoints)
        {
            result = target_remove_breakpoint (&bp_tgt2);
            if (result != 0)
                warning ("Can not remove breakpoint at 0x%08X\n",  (unsigned int) bp_tgt2.placed_address);
        }
    }
...
static void
arc_linux_write_pc (CORE_ADDR pc, ptid_t ptid)
{
