...
arc_software_single_step(enum target_signal ignore, int insert_breakpoints_p)
{
  ...
  static unsigned char break_mem[2][16];

  static char two_breakpoints;
  CORE_ADDR pc;
  static struct bp_target_info bp_tgt1;
  static struct bp_target_info bp_tgt2;
  {
#ifdef ARC_DEBUG
  ...
  if (insert_breakpoints_p)
    {
        memset (&bp_tgt1, 0, sizeof (struct bp_target_info));
        memset (&bp_tgt2, 0, sizeof (struct bp_target_info));
        bp_tgt1.shadow_contents[0] =  break_mem[0];
        bp_tgt1.shadow_contents[1] =  break_mem[1];
        bp_tgt2.shadow_contents[0] =  break_mem[0];
        bp_tgt2.shadow_contents[1] =  break_mem[1];
    	...
    	if (two_breakpoints && branch_target == pc)
		{
		 ...
		 //printf("irfan: arc_software_single_step 1");
    	 bp_tgt1.placed_address = fall_thru;
    	 target_insert_breakpoint (&bp_tgt1);
		 fall_thru = bp_tgt1.placed_address;
    	 if(two_breakpoints)
    	 {
			bp_tgt2.placed_address = branch_target;
			target_insert_breakpoint (&bp_tgt2);
		    branch_target=bp_tgt2.placed_address;
		 }
    	}
  	else
  	  {
  	    //printf("irfan: arc_software_single_step 2");
  	    // <irf-todo> target_remove_breakpoint (fall_thru, break_mem[0]);
  	    bp_tgt1.placed_address = fall_thru;
  	    target_remove_breakpoint (&bp_tgt1);
		fall_thru=bp_tgt1.placed_address;
  	    if(two_breakpoints)
  	    {
			// <irf-todo> target_remove_breakpoint (branch_target, break_mem[1]);
			bp_tgt2.placed_address = branch_target;
			target_remove_breakpoint (&bp_tgt2);
		    branch_target = bp_tgt2.placed_address;
		}
  	  }
#endif
