...
register_reggroup_p (int regnum, struct reggroup *group)
{
    ...
    /* Build configuration registers */
    if (ARC_BCR_0_REGNUM <= regnum && regnum <= ARC_BCR_1F_REGNUM)
    {
        /* some are not used (i.e. the corresponding "slot" in the Auxiliary
         * Register space is not assigned to a register).
         */
        if (group == all_reggroup)
            return arc_BCR_is_used(regnum) ? 1 : 0;

        /* BCRs are read-only, so are neither saved nor restored */
